...
# gym's pendulum default values
# control the random range the pendulum can start at
# DEFAULT_X = np.pi 
# DEFAULT_Y = 1.0

# both being 0 is straight up and not moiving
# for pendulum_base, very small range 
# DEFAULT_X = np.radians(2) # range for starting theta
# DEFAULT_Y = 0.01 # range for starting thetadot

# can start at any location, at any speed
...
class PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, g=9.81):
        ...
        #   or normalised as max_deflect == 2 by default. Ignoring the issue here as the default settings are too old
        #   to update to follow the gymnasium api
        self.action_space = spaces.Box(
            low=np.array([-self.max_deflect]), high=np.array([self.max_deflect]), shape=(1,), dtype=np.float32
        )
    ...
    def step(self, u):
        ...
        # self.trace.append([np.rad2deg(th), np.rad2deg(thdot), u])
